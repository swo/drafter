...
class DraftGroup:
    def __init__(self, source, drafts):
        self.source = source
        self.drafts = sorted(drafts, key=lambda x: (x.date, x.version))

    def next_draft_path(self):
        if self.drafts[-1].date == date.today().isoformat():
            return make_draft_path(self.drafts[-1].name, version=self.drafts[-1].version + 1)
